...
const [getConfig1, setConfigMatcher] = mod3.getConfigAndMatcher({
    alias: {
        type: "string",
        name: "Alias",
...
const health = {
    async "web-ui" (effects, duration) {
        return mod4.checkWebUrl("http://utxoracle.embassy:80")(effects, duration).catch(mod4.catchError(effects));
    }
};
    ...
    bitcoind: {
        async check (effects, configInput) {
            ...
            if (matchOldBitcoindConfig.test(configInput) && configInput.advanced.pruning.mode !== "disabled") {
                return {
                    error: 'Pruning must be disabled.'
                };
            }
            return {
                result: null
